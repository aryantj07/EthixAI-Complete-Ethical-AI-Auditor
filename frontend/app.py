...
# Generate PDF report with tables and charts from existing session_state metrics
def generate_report_pdf():
    buffer = io.BytesIO()
    with PdfPages(buffer) as pdf:
        # Page 1: Performance comparison table as text
        fig, ax = plt.subplots(figsize=(8,6))
        ax.axis('tight')
        ax.axis('off')
        perf_compare = st.session_state.get("performance_compare", pd.DataFrame())
        if not perf_compare.empty:
            table_data = perf_compare.round(4).to_string(index=True)
        else:
            table_data = "No performance comparison data available."
        ax.text(0, 1, "Performance Comparison Before and After Debiasing\n\n" + table_data,
                verticalalignment='top', fontfamily='monospace', fontsize=10)
        pdf.savefig(fig)
        plt.close(fig)

        # Page 2: Bar chart for selected metrics
        fig, ax = plt.subplots(figsize=(8, 6))
        if not perf_compare.empty:
            metrics_to_plot = [col for col in perf_compare.columns if col != 'Model']
            x = range(len(metrics_to_plot))
            raw_vals = perf_compare.loc['Baseline (Raw)'][metrics_to_plot].values
            deb_vals = perf_compare.loc['After Debiasing'][metrics_to_plot].values

            ax.bar([i - 0.2 for i in x], raw_vals, width=0.4, label='Raw')
            ax.bar([i + 0.2 for i in x], deb_vals, width=0.4, label='Debiased')
            ax.set_xticks(x)
        ...
        # Page 3: Overall Scores summary
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.axis('off')
        raw_score = st.session_state.get("raw_metrics", {})
        deb_score = st.session_state.get("debiased_metrics", {})
        # Example using Accuracy if available, else sum of all metrics:
        raw_acc = raw_score.get('Accuracy', None)
        deb_acc = deb_score.get('Accuracy', None)
        raw_f1 = raw_score.get('F1', None)
        deb_f1 = deb_score.get('F1', None)

        lines = "Overall Model Scores\n\n"
        if raw_acc is not None and deb_acc is not None:
            lines += f"Baseline (Raw) Accuracy: {raw_acc:.4f}\n"
            lines += f"Debiased Accuracy: {deb_acc:.4f}\n\n"
        if raw_f1 is not None and deb_f1 is not None:
            lines += f"Baseline (Raw) F1 Score: {raw_f1:.4f}\n"
            lines += f"Debiased F1 Score: {deb_f1:.4f}\n\n"

        ax.text(0, 1, lines, verticalalignment='top', fontsize=12, fontweight='bold')
        pdf.savefig(fig)
        plt.close(fig)
...
if st.session_state.get("performance_compare") is not None:
    st.markdown("---")
    st.write("### Download Report")
    pdf_data = generate_report_pdf()
    st.download_button(
        label="Download Performance & Fairness Report (PDF)",
        data=pdf_data,
...
def run_simulation(df, target_col, sensitive_col, bias_strength, sample_size=5000):
...
def interpret_metric(name, value):
    ...
    if abs_val <= 0.05:
        level = "Low bias (good)"
        advice = "Indicates fairly equal outcomes between groups."
    elif abs_val <= 0.15:
        level = "Moderate bias"
        advice = "Some disparity exists; consider reviewing model fairness."
    else:
        level = "High bias (needs attention)"
        advice = "Significant disparity detected, bias mitigation recommended."
    return f"**{name}:** {value:.3f} — {level}\n\n{advice}"
...
if st.session_state.page == "Simulator":
    ...
    if df is None:
        ...
        st.markdown(
            "Bias strength controls how much predictions favor one sensitive group over another: "
            "**0** means no bias, positive values favor the first group, "
            "negative values favor the second group."
        )

        if simulator is None:
            st.info("Simulator backend missing.")
        else:
            simulate_btn = st.button("Simulate Bias Effect")

            if simulate_btn:
                target_guess = st.session_state.get("deb_target") or (df.columns[-1] if len(df.columns) else None)
                sens_guess_candidates = detect_sensitive_attributes(df)
                sens_guess = st.session_state.get("deb_sensitive") or (sens_guess_candidates[0] if sens_guess_candidates else (df.columns if len(df.columns) else None))

                try:
                    target_series = df[target_guess if target_guess else df.columns[-1]]
                    if target_series.dtype == object or not pd.api.types.is_numeric_dtype(target_series):
                        le = sklearn.preprocessing.LabelEncoder()
                        target_numeric = le.fit_transform(target_series.fillna("missing"))
                    else:
                        target_numeric = target_series.fillna("missing").astype(int).values

                    df_sim = df.copy()
                    df_sim[target_guess if target_guess else df.columns[-1]] = target_numeric

                    preds_sim, metrics_sim = run_simulation(
                        df_sim,
                    ...
                    # Display raw metrics dict
                    st.write(metrics_sim)

                    # Display user-friendly interpretations
                    if "Demographic Parity Difference" in metrics_sim:
                        st.markdown(interpret_metric("Demographic Parity Difference", metrics_sim["Demographic Parity Difference"]))

                    if "Equalized Odds Difference" in metrics_sim:
                        st.markdown(interpret_metric("Equalized Odds Difference", metrics_sim["Equalized Odds Difference"]))

                except Exception as e:
                    st.error(f"Simulator failed: {e}")
            else:
                st.info("Adjust the bias strength and click 'Simulate Bias Effect' to run the simulation.")
...
if st.session_state.page == "Drift":
    ...
    if df is None:
        ...
        st.markdown(
            "Data drift occurs when the statistical properties of data change over time, which can cause "
            "a model’s performance to degrade. This tool lets you check for drift by comparing a previous "
            "model performance score with a current score."
        )
        ...
        if st.button("Check Drift"):
            if drift is None:
                ...
                try:
                    ...
                    if drifted:
                        st.warning("Warning: Significant data drift detected. Consider retraining the model.")
                    else:
...
if st.session_state.page == "Community":
    ...
    st.write(
        "Browse submissions, preview attachments including PDFs and datasets, and see before/after scores if available. "
        "Cast your votes and contribute your own insights or data."
    )
    ...
    if community is None:
        ...
        # Helper for dataset preview
        def load_dataset_preview(filename):
            filepath = Path(BACKEND_PATH) / "data" / filename
            ...
            if subs.empty:
                ...
                for col in ['upvotes', 'downvotes', 'submission_id']:
                    if col not in subs.columns:
                        subs[col] = [f"fallback_{i}" if col == 'submission_id' else 0 for i in range(len(subs))]
                ...
                total_upvotes = subs['upvotes'].sum()
                total_downvotes = subs['downvotes'].sum()
                st.markdown(f"**Total Upvotes:** {total_upvotes} | **Total Downvotes:** {total_downvotes}")
                st.markdown("---")
                    ...
                    with st.container():
                        ...
                        with vote_col:
                            st.button(
                                ...
                                help="Upvote this submission"
                            )
                            st.markdown(
                                f"<div style='text-align:center; color:#2D58A0; font-weight:bold;'>{row['score']}</div>",
                                unsafe_allow_html=True
                            )
                            st.button(
                                "👎",
                        ...
                        with content_col:
                            ...
                            attached_file = row.get('attached_filename', None)
                            if attached_file and attached_file not in [None, ""]:
                                st.info(f"Attachment included: `{attached_file}`")
                                if attached_file.lower().endswith('.pdf'):
                                    filepath = Path(BACKEND_PATH) / "data" / attached_file
                                    try:
                                        ...
                                        # Embed PDF preview
                                        pdf_display = pdf_bytes.encode("base64").decode()
                                        pdf_html = f'<iframe src="data:application/pdf;base64,{pdf_bytes.encode("base64").decode()}" width="700" height="400" type="application/pdf"></iframe>'
                                        st.components.v1.html(
                                            pdf_html,
                                            height=400,
                                        )
                                    except Exception:
                                        st.write("Unable to preview PDF.")
                                else:
                                    preview = load_dataset_preview(attached_file)
                                    if preview is not None:
                                        st.write("Dataset Preview (first 5 rows):")
                                        st.dataframe(preview)
                                    else:
                                        st.write("Preview unavailable for this attachment.")

                            before_score = row.get('before_score', None) or row.get('before', None)
                            after_score = row.get('after_score', None) or row.get('after', None)
                            if before_score is not None and after_score is not None:
                                st.markdown(f"**Before Score:** {before_score} | **After Score:** {after_score}")

        except Exception as e:
            st.error(f"Could not load submissions. Error: {e}")

        # Submission form
        st.markdown("---")
        st.write("### Submit your contribution")
        with st.form("submission_form", clear_on_submit=True):
            ...
            content = st.text_area("Describe your submission (required)", placeholder="Provide details about your submission...")
            attached = st.file_uploader("Attach a file (optional) [CSV, XLSX, JSON, TXT, PDF]", type=["csv", "xlsx", "json", "txt", "pdf"])
            ...
            if submitted:
                ...
                try:
                    ...
                    st.success("Successfully submitted! Please refresh to see your post.")
                except Exception as e:
